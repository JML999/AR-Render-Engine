...
public class SceneRenderer {
    ...
    private var quadDepthStencilState: MTLDepthStencilState
    private var sceneDepthStencilState: MTLDepthStencilState
    ...
    public static func makeDepthStencilState(device: MTLDevice, depthReadEnabled: Bool, depthWriteEnabled: Bool)->MTLDepthStencilState{
    ...
    private func textureForMaterialProperty(_ property: MaterialProperty) -> MTLTexture? {
        ...
        if let textureContents = property.contents as? MTLTexture {
            ...
            fatalError("Could not understand type of material property contents")
        }
    ...
    public func drawNode(_ node: Node, viewMatrix: float4x4, pass: MTLRenderPassDescriptor){
        ...
        for element in geometry.elements {
            ...
            renderCommandEncoder.drawIndexedPrimitives(type: element.primitiveType, indexCount: element.indexCount, indexType: element.indexType, indexBuffer: element.indexBuffer, indexBufferOffset: element.indexBufferOffset)
        }
    }
    
    public func drawVideoQuad(_ frame: ARFrame, viewPortSize: CGSize, pass: MTLRenderPassDescriptor){
        ...
        if let luma = lumaTexture, let chroma = chromaTexture {
            let renderPipelineState = shaderManager.fullScreenQuadPipelineState(pass: pass)
